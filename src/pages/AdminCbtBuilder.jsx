...
export default function AdminCbtBuilder() {
  ...
  const courseOptions = useMemo(() => {
    ...
    if (filterDepartment)
      list = list.filter((c) => c.department === filterDepartment);
  ...
  async function handleSubmit(e) {
    ...
    if (!form.courseId || !selectedCourse) {
      return setError(
        "Select a course from the list. Add courses under Admin → Courses first."
      );
    }
    ...
    try {
      const payload = {
        ...
        hintEnabled: Boolean(form.hintEnabled),
      };
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-secondary">
            Questions power the new Practice & Timed CBT workspace. Manage courses
            under <span className="text-accent">Admin → Courses</span>.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {/* AI entry points — keep your existing modals wired here */}
          <button
           type="button"
          onClick={() => setShowAiGenerate(true)}
          className="flex items-center gap-2 rounded-lg border border-accent/40 bg-accent-soft px-4 py-2 text-sm font-medium text-accent hover:bg-accent/15"
>
  <Sparkles size={16} />
  Generate with AI
</button>
      ...
      <AiGenerateQuestionsModal
       ...
       courses={courses}
        onSaved={() => {
    // list already refreshes inside the modal via refreshCbtQuestions()
    // optional: toast or keep form closed
  }}
/>
        ...
        >
          ...
          {courses.length === 0 && (
            <p className="rounded-lg border border-status-warning/40 bg-status-warning/10 px-3 py-2 text-sm text-status-warning">
              No courses yet. Go to <strong>Admin → Courses</strong> and add the
              official list first.
            </p>
          )}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div>
              <label className="mb-1 block text-xs text-text-muted">Marks</label>
              <select
                value={form.marks}
                onChange={(e) =>
          ...
          <div className="flex items-center gap-2">
            <input
              id="hintEnabled"
              type="checkbox"
              checked={form.hintEnabled}
              onChange={(e) =>
                setForm({ ...form, hintEnabled: e.target.checked })
              }
              className="h-4 w-4 rounded border-border-subtle text-accent focus:ring-accent"
            />
            <label htmlFor="hintEnabled" className="text-sm text-text-primary">
              Enable practice-mode hint (shows explanation drawer in CBT)
            </label>
          </div>
          ...
          <div>
            <label className="mb-1 block text-xs text-text-muted">
              Explanation / Hint (shown after submit or in practice mode)
            </label>
            <textarea
              ...
              rows={2}
              placeholder="Step-by-step reasoning, theorem used, common mistakes…"
              className={fieldClass}
            />
          </div>
